...
class Scheduler {
  ...
  push (future) {
    // Could be redundant, but not worth deduplicating
...
class Future {
  ...
  map (fun) {
    ...
    pairMapped(this, mappedInto, fun)
    return mappedInto
  }
  ...
  static all (values) {
    ...
    const pendingFutures = values.map(function initJoinedToAll (value, index) {
      const pending = this.fromResult(value).map(function joinedToAllMapper (error, result) {
  ...
  static race (values) {
    ...
    const pendingFutures = values.map(function initJoinedToRace (value) {
      const pending = this.fromResult(value).map(function joinedToRaceMapper (error, result) {
        pull(pendingFutures, pending)
...
function unpairMapped (parent, child) {
  parent.mappedInto_ = undefined
  child.mappedFrom_ = undefined
}

function pairMapped (parent, child, mapper) {
  child.mappedFrom_ = parent
  child.mapper_ = mapper
  parent.mappedInto_ = child
  parent.consumed_ = true
  parent.pendingRejection_ = false
  if (parent.state !== parent.states.PENDING) {
    parent.constructor.scheduler.push(parent)
  }
}
...
function flushBy (array, fun) {
  try {
    while (array.length) fun(array.pop())
  }
  catch (err) {
    flushBy(array, fun)
    throw err
