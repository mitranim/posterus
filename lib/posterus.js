...
class Scheduler {
  ...
  tick() {
    ...
    // Ensures exception resilience without obfuscating the throw site too much
    try {
...
class Future {
  ...
  settle(error, result) {
    ...
    if (isFuture(error)) {
      ...
      if (isBaseFuture(error) && !error.successor_ && !this.mapper_) {
    ...
    if (isFuture(result)) {
      ...
      if (isBaseFuture(result) && !result.successor_) {
  ...
  mapError(mapper) {
    validate(isFunction, mapper)
    return this.map(mapToError.bind(null, mapper))
  }

  mapResult(mapper) {
    validate(isFunction, mapper)
    return this.map(mapToResult.bind(null, mapper))
  }
  ...
  toPromise() {
    ...
    return new Promise(mapFutureToPromise.bind(null, this))
  }
...
exports.ignoreDeinitError = ignoreDeinitError
function ignoreDeinitError(error, result) {
  if (error) {
    if (error.message === deinitErrorMessage) return undefined
    throw error
  }
  return result
}
