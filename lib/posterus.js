...
// Using bitwise instead of multiple boolean fields saves ≈25% memory (in V8)
// at the cost of REPL convenience. Not sure if worth it.
const _UNSET_ALL        = 0b00000000
const PENDING           = 0b00000001
const ERROR             = 0b00000010
const SUCCESS           = 0b00000100
const AVERTED           = 0b00001000
const PENDING_REJECTION = 0b00010000
const CONSUMED          = 0b00100000
const MAPPING           = 0b01000000
const SET_ALL           = 0b11111111
    ...
    for (let i = -1; ++i < values.length;) {
      ...
      if (value instanceof Future) {
        if (someSet(value, ERROR)) {
          unset(value, PENDING_REJECTION)
          set(value, CONSUMED)
          this.arriveAtIndex_(i, value.value_)
          return
        }

        if (someSet(value, SUCCESS)) {
          set(value, CONSUMED)
          values[i] = value.value_
          continue
        }

        if (!value.mapper_) {
          set(value, CONSUMED)
          value.mapper_ = this.arriveAtIndex_.bind(this, i)
          continue
        }
      }
...
class RaceJunction {
  constructor (future, racing) {
    this.future_ = future
    this.racing_ = racing
    future.deiniter_ = this.deinit_.bind(this)

    for (let i = -1; ++i < racing.length;) {
      const value = racing[i]
      ...
      if (value instanceof Future) {
        ...
        if (someSet(value, SUCCESS)) {
          set(value, CONSUMED)
          this.arriveAtIndex_(i, undefined, value.value_)
          return
        }
      ...
      racing[i] = value.map(this.arriveAtIndex_.bind(this, i))
    }
  }

  arriveAtIndex_ (index, error, result) {
    if (this.racing_) {
      this.racing_[index] = undefined
  ...
  deinit_ () {
    const racing = this.racing_
    this.racing_ = undefined
    if (racing) forceDeinitMaybeFutures(racing)
  }
}
...
function unlinkPair (parent, child) {
  parent.nextInChain_ = undefined
  child.prevInChain_ = undefined
}

function linkPair (parent, child, mapper) {
  child.prevInChain_ = parent
  child.mapper_ = mapper
  parent.nextInChain_ = child
  set(parent, CONSUMED)
  unset(parent, PENDING_REJECTION)
  if (noneSet(parent, PENDING)) scheduleFuture(parent)
}
