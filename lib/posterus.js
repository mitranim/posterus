...
class Future {
  ...
  weak () {
    ...
    if (isSomeSet(this, ERROR)) return this.constructor.fromError(this.value_)
    if (isSomeSet(this, SUCCESS)) return this.constructor.fromResult(this.value_)
...
function arriveWithCaughtError (future, err) {
  if (isSomeSet(future, PENDING)) future.arrive(err)
  else throw err
}
