...
class Future {
  ...
  mapResult (fun) {
    ...
    validate(isFunction, fun)
    return this.map(mapFinally.bind(null, fun))
  }
...
function mapFinally (fun, error, result) {
  const finalisation = fun(error, result)
  if (isFuture(finalisation)) {
    return finalisation.mapResult(function resetResult () {return result})
  }
  if (error) throw error
  return result
}
