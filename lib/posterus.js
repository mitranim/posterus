...
class Reactor {
  constructor () {
    ...
    this.pendingTick_ = false
    this.tickScheduled_ = reactorTickScheduled.bind(null, this)
    this.asap = asap(this.tickScheduled_)
  }

  push (future) {
    // Not worth deduplicating
    this.pending_.push(future)
    if (this.pending_.length) scheduleReactorTick(this)
  }
...
// TODO document the ability to synchronously finish pending operations by
// calling Reactor.global.tick(). Should also use that in tests.
Reactor.global = new Reactor()

function scheduleReactorTick (reactor) {
  if (!reactor.pendingTick_) {
    reactor.pendingTick_ = true
    reactor.asap.call(null, reactor.tickScheduled_)
  }
}

function reactorTickScheduled (reactor) {
  reactor.pendingTick_ = false
  reactor.tick()
}
...
class Future {
  ...
  map (fun) {
    ...
    this.pendingRejection_ = false
    if (this.state !== this.states.PENDING) Reactor.global.push(this)
