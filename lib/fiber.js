...
function fiber(iter) {
  ...
  function onDone(error, value) {
    if (error) out.settle(error)
    else if (isIterator(value)) out.settle(undefined, fiber(value))
    else out.settle(undefined, value)
  ...
  function onIterNext({value, done}) {
    ...
    if (isPromise(value)) {
      pending = Future.fromPromise(value).map(onPendingDone)
      return
    }
